...
let u=0;
$(function(){
	$('.video_reply_reply').click(function(){
	  $('.rIn').hide();
	  $('.rUp').hide();
	  $('.video_reply_reply').text("대댓글");
	...
	$('.video_reply_update').click(function(){
		let no=$(this).attr("value");
		$('.rIn').hide();
		$('.rUp').hide();
		if(u==0)
...
<article class="viewPage">
  <article class="viewAtc">
    <div class="infoBx">
	    <h3 class="tit">${vo.v_title }</h3>
	     <p class="infoTx">
	      <span class="cate">${vo.v_writer }</span>
	      <span class="txViewNum"><i class="glyphicon glyphicon-user" style="color:#336699"></i>&nbsp;${vo.v_hit }
	      <div style="height:5px"></div>
	      </span>
	     </p>
	      <div class="funcBtn" style="float: right;">
	      <c:if test="${count==0 }">
	       <button type="button" class="btnPrint" title="스크랩" onclick="location.href='videoScrapInsert.do?no=${vo.v_no }'"><span><i class="glyphicon glyphicon-thumbs-up"></i> 스크랩</span></button>
	       </c:if>
    ...
    <div id="dialog" title="링크 주소" style="display:none; background-color:white; border:1px solid #e6e6e6; text-align:center;">
     <p>http://localhost:8080/Team4thProject/<br>newsTipVideo/videodetail.do?no=${vo.v_no }</p>
    </div>
    
<center><iframe id="player" type="text/html" width="640" height="360" src="${vo.v_video }" frameborder="0"></iframe></center>
<br><br>
 <button type="button" title="목록" style="float: right;" onclick="location.href='../newsTipVideo/video.do'"><span>돌아가기</span></button>
</article>
</article>
</div>
...
<div class="col-lg-12">
      ...
      <table class="table">
        <tr>
          ...
          <td class="text-right">
            ...
            <button type="button" style="background-color:#99ffc9; border:1px solid #bfbfbf; border-radius:5px; padding:5px 5px" class="video_reply_update" title="수정" value="${rvo.no }"><span>수정</span></button>
            <button type="button" style="background-color:#ffd6cc; border:1px solid #bfbfbf; border-radius:5px; padding:5px 5px" onclick="location.href='../newsTipVideo/video_reply_delete.do?no=${rvo.no }&bno=${vo.v_no}'" title="삭제"><span>삭제</span></button>
            </c:if>
            <button type="button" style="background-color:#ccd8ff; border:1px solid #bfbfbf; border-radius:5px; padding:5px 5px" class="video_reply_reply" title="대댓글" value="${rvo.no }"><span>대댓글</span></button>
          </td>
        </tr>
        <tr>
       ...
       <td colspan="2">
        <form method=post action="../newsTipVideo/video_reply_reply_insert.do">
            <input type=hidden name="no" value="${rvo.no }">
            <input type=hidden name="bno" value="${vo.v_no }">
	        <textarea rows="3" cols="80" name="msg" style="float: left; width:800px; height:100px;"></textarea>
	        <button type="submit" value="대댓글달기" style="background-color:#ccd8ff; border:1px solid #bfbfbf; border-radius:5px; padding:38px 30px; margin-left:7px;"><span>댓글달기</span></button>
        </form>
       </td>
      </tr>
      <tr class="rUp" id="rUp${rvo.no }" value="${rvo.no }" style="display:none">
       <td colspan="2">
        <form method=post action="../newsTipVideo/video_reply_update.do">
            <input type=hidden name="no" value="${rvo.no }">
            <input type=hidden name="bno" value="${vo.v_no }">
	        <textarea rows="3" cols="80" name="msg" style="float: left; width:800px; height:100px;">${rvo.msg }</textarea>
	        <button type="submit" value="댓글수정" style="background-color:#99ffc9; border:1px solid #bfbfbf; border-radius:5px; padding:38px 30px; margin-left:7px;"><span>수정하기</span></button>
        </form>
       </td>
      </tr>
      </table>
    ...
    <table class="table">
      <tr>
       <td>
        <form method=post action="../newsTipVideo/video_reply_insert.do">
            <input type=hidden name="bno" value="${vo.v_no }">
